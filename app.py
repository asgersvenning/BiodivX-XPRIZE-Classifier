...
def postprocess(predictions):
    return ([f'{c} ({cell_formatter(s, digits=1)})' for c, s in zip(predictions["class"], predictions["score"])], [dict2csv({k : [v[i]] for k, v in predictions["data"].items()}) for i in range(len(predictions["data"]["Predicted"]))])
...
with gr.Blocks() as demo:
    ...
    rich_output = gr.Textbox(label="Rich Output", type="text")
